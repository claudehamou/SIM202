...
class Individu  //classe virtuelle
{
public:
    ...
    virtual Individu& flip() = 0; //fonction de mutation virtuelle

    virtual Individu* initialise_indiv() = 0; // fonction d'initialisation d'individu virtuelle pure

    virtual Individu* clone() = 0;   // fonction de clonage virtuelle

    virtual pair<Individu*, Individu*> crossover(const Individu& ind_2) = 0; //fonction d'hybridation virtuelle
...
// fonctions de selection de population finale (mix entre parents et enfants)
...
Individu* algo_genetique(int,double,typeselection,typefinale,int,Individu*,int); //fonction algorithme genetique
