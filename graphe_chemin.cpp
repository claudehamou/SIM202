...
Chemin::Chemin()
{
    Graphe G = Graphe(3); //initialise a un graphe arbitraire de 3 villes
    G(0, 0) = -1;
    G(0, 1) = 2;
    G(0, 2) = 5;
    G(1, 0) = 2;
...
Chemin::Chemin(Graphe* G)
{
    ...
    for (int k = 0; k < G->nbvilles; k++) { villes.at(k) = k; }  //initialisation au chemin dans l'ordre des indices des villes /!\ ne prend pas en compte les '-1'
}

Chemin& Chemin::operator = (const Chemin& C) //operateur =
{
    graphe = C.graphe;
...
double Chemin::adaptation()  //renvoie la fonction d'adaptation du chemin, i.e. sa distance
{
    double adapt = 0;
    if (this==0) {return 0;}
    int n = villes.size();
...
pair<Individu*, Individu*> Chemin::crossover(const Individu& i_1)
{
    ...
    int l = rand() % (C->villes.size() - 1);    //on tire l entre 0 et n-1
    for (int k = 0; k < l + 1; k++)
    {
    ...
    for (unsigned short int k = l + 1; k < (unsigned short)(C->villes.size()); k++)
    {
        ...
        if (d2 == 0)
        {
            C->villes.at(k - cd) = villes.at(k); //remplir les villes restantes du chemin en plus si pas deja dans C
        }
        cd += d2;
        j = 0;
        while (d3 == 0 && j < l + 1)
        ...
        if (d3 == 0)
        {
            D->villes.at(k - dd) = C1.villes.at(k); //remplir les villes restantes de C1 en plus si pas deja dans D
        }
        dd += d3;
    }
    while (cd != 0)
    ...
    while (dd != 0)
    {
        int rep = 0;
        int i = 0;
        while (rep == 0) //reperer les indices de villes qui ne sont pas dans D
        {
            unsigned short int k = 0;
...
Chemin& Chemin::flip()
{
    int l = (rand() % (villes.size() - 1)) + 1;     //on prend l au hazard entre 1 et n
    int k = rand() % (villes.size() - 1);           //on prend k au hazard entre 0 et n-1
    int ck = villes.at(k);
    int ck2 = villes.at(k + 1);
    int cl = villes.at(l);
    int cl2 = villes.at(l - 1);
    villes.at(l) = ck;                              //on echange les paires de villes concernees
    villes.at(l - 1) = ck2;
    villes.at(k + 1) = cl2;
    villes.at(k) = cl;
    return *this;
...
Chemin* Chemin::initialise_indiv()  //on initialise le chemin de proche en proche : voisin = ville la plus proche non deja choisie
{
